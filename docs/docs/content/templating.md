...
| `{{ TrackLink "https://link.com" }}` | Takes a URL and generates a short public tracking URL (`/l/...`) using PocketBase record ids for the link, campaign, and subscriber (legacy `/link/...` URLs with UUIDs still work for older messages). |
