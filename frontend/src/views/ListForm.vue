      ...
      <header class="admin-dialog-head modal-card-head">
        <div class="dialog-meta-row">
          <p v-if="isEditing" class="entity-meta has-text-grey-light is-size-7">
            ...
            Record ID: <copy-text :text="data.recordId || data.record_id || data.id" />
...
export default {
  ...
  methods: {
    ...
    updateList() {
      this.$api.updateList({ id: this.data.id, record_id: this.data.recordId || this.data.record_id || this.data.id, ...this.form }).then((data) => {
        this.$emit('finished');
        this.$emit('close');
